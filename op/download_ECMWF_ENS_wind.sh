...
# bash download_ECMWF_ENS_wind.sh 00 1 /home/ec2-user/SageMaker/work/data/ECMWF/wind/00 8
...
echo "Downloading ECMWF wind ensemble from S3: date=$DATE cycle=${HCYCLE}z"

S3_PATH="s3://ecmwf-forecasts/${DATE}/${HCYCLE}z/ifs/0p25/enfo/"
...
for f in "$DOWNLOAD_DIR"/*-ef.grib2; do
  [[ -f "$f" ]] || continue
  base=$(basename "$f")
  # Extract: YYYYMMDDHH0000-Xh-enfo-ef.grib2 -> YYYYMMDDHH-XXXh-enfo-ef.grib2
