...
PURPOSE:
 ...
 The goal is to provide a spatial validation on the size/position of the polygons, predicted versus occur
ed.
...
USAGE:
 ...
 Example (from linux terminal command line):
  python3 probmaps_gefs_valdt.py probmaps_gefs_internal.yaml 2023060900 7 14 Hs
  nohup python3 probmaps_gefs_valdt.py probmaps_gefs_internal.yaml 2023060900 7 14 Hs >> nohup_probmaps_g
efs.out 2>&1 &
...
def build_gefs_hindcast(date,mvar,auxftime,path_gefs):

    # cycle time
    ctime=np.array(np.arange(auxftime.min(),auxftime.max()+1,24)*3600.+timegm( strptime(date,'%Y%m%d%H') )).astype('double')
    ...
    for ct in range(0,len(ctime)):
        ...
        for t in range(0,auxlt.shape[0]):
            for enm in range(0,nenm):
                ...
                fname=gefspath+cdate+"00/gefs.wave."+cdate+"."+str(int(enm)).zfill(2)+".global.0p25.f"+str(int(auxlt[t])).zfill(3)+".grib2"
                ...
                if c==0:
                    ...
                    if 'Significant' in mvar:
                        xrvar='swh'
                    else:
                        xrvar='ws'

                ds = xr.open_dataset(fname, engine='cfgrib')
                fmod[tt,enm,:,:] = np.array(np.flip(ds[xrvar].values[:],axis=0)).astype('float')
                ds.close(); del ds
...
if __name__ == "__main__":
    ...
    for t in range(0,auxltime.shape[0]):
        for enm in range(0,nenm):
            ...
            fname=gefspath+fcdate+"00/gefs.wave."+fcdate+"."+str(int(enm)).zfill(2)+".global.0p25.f"+str(int(auxltime[t])).zfill(3)+".grib2"
            if c==0:
                ds = xr.open_dataset(fname, engine='cfgrib')
                wtime = np.atleast_1d(np.array(ds.time.values))
                lat = np.array(ds.latitude.values); lat = np.sort(lat); lon = np.array(ds.longitude.values)
    ...
    print(" 2. Build reference ...")
    # Read GEFS and build 24-h slices (Hindcast)
    auxftime = np.arange((ltime1-1)*24,((ltime2)*24)+1,tres)
    hfmod, hlat, hlon, hwtime = build_gefs_hindcast(fcycle,mvar,auxftime,gefspath)
    hfmod = np.copy(hfmod[:,:,indlat[0],:][:,:,:,indlon[0]])
    hfmod = np.nanmean(np.sort(hfmod,axis=0)[-nmax::,:,:,:],axis=0)
    hfmod=hfmod*umf
    hfmod[hfmod>200]=np.nan
    # Bias correction module - QMM (to be implemented)
    bc_slope = 1.05; bc_intercept = 0.
    hfmod = hfmod*bc_slope + bc_intercept

    del indlat, indlon, hlat, hlon, hwtime
    print(" 2. Build reference ... OK"); print(" ")
    ...
    for i in range(0,qlev.shape[0]):
        ...
        for j in range(0,nenm):
            csv = ax.contour(lon, lat, hfmod[j,:,:], levels=[qlev[i]],colors='dimgrey',alpha=0.7,linewidths=1,zorder=2,transform=ccrs.PlateCarree())

        # hatch = ax.contourf(lon,lat,gaussian_filter(np.nanmean(hfmod,axis=0), gft),levels=[9.0, 1e6],colors="gray",alpha=0.5,hatches=["//"],linewidths=0.5,transform=ccrs.PlateCarree(),zorder=3)
        csv = ax.contour(lon, lat, np.nanmean(hfmod,axis=0), levels=[qlev[i]],colors='k',linewidths=1.5,zorder=2,transform=ccrs.PlateCarree())
        ...
        figname = outpath+"ProbMap_SpatialValidation_"+fcycle[0:8]+"_"+fvarname+"_"+str(qlev[i]).zfill(1)+"_fcst"+pd.to_datetime(wtime[0] + np.timedelta64(ltime1, 'D')).strftime('%Y%m%d')+"to"+pd.to_datetime(wtime[0] + np.timedelta64(ltime2, 'D')).strftime('%Y%m%d')+"_"+ftag
